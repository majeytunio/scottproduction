...
//       status: getColumnIndex(['Status', 'Deal Status', 'Approval Status']),
//       monthlyPremium: getColumnIndex(['Monthly Premium', 'MonthlyPremium', 'Premium']),
//       commissionPercent: getColumnIndex(['Agent Commission %', 'Commission %', 'Commission Rate', 'Rate']),
//       annualCommission: getColumnIndex(['Annual Commission $', 'Annual Commission', 'Total Commission']),
//       netAdvance: getColumnIndex(['Net Advance (60%)', 'Net Advance', 'Agent Net']),
//       grossCommission: getColumnIndex(['Gross Commission', 'Annual Commission $']),
//       coverageAmount: getColumnIndex(['Coverage Amount', 'Coverage', 'Loan Amount']),
//       product: getColumnIndex(['Product', 'Product Type', 'Insurance Type']),
//     }
...
//       // Get commission percentage from spreadsheet
//       let commissionPercent = 110
//       if (columnIndexes.commissionPercent !== -1 && row[columnIndexes.commissionPercent]) {
//         const percentStr = row[columnIndexes.commissionPercent]
//         const percentNum = parseFloat(percentStr.replace(/[^\d.-]/g, ''))
...
//       // Get GROSS commission from spreadsheet (Annual Commission $ column)
//       let grossCommission = 0
//       if (columnIndexes.annualCommission !== -1 && row[columnIndexes.annualCommission]) {
//         grossCommission = parseFloat(row[columnIndexes.annualCommission].replace(/[^\d.-]/g, '')) || 0
//       }
      
//       // If no gross commission in column, calculate it
//       if (grossCommission === 0) {
//         grossCommission = annualPremium * (commissionPercent / 100)
//       }
//       obj._grossCommission = String(grossCommission)
      
//       // Get NET commission from spreadsheet (Net Advance 60% column) - MOST IMPORTANT
//       let netCommission = 0
//       if (columnIndexes.netAdvance !== -1 && row[columnIndexes.netAdvance]) {
//         netCommission = parseFloat(row[columnIndexes.netAdvance].replace(/[^\d.-]/g, '')) || 0
//       }
      
//       // If no net commission in column, calculate it from gross commission × 60%
//       if (netCommission === 0) {
//         netCommission = grossCommission * 0.60 // Apply 60% split (0.75 × 0.80)
//       }
//       obj._netCommission = String(netCommission)
...
//       netCommission: 225000, // Net commission goal (60% of gross)
//       production: 400000,    // Annual production goal
//       deals: 400,            // Deal count goal
//       dailyTarget: 2000      // Daily gross commission target
//     }
...
async function getGoogleSheetData() {
  try {
    ...
    const columnIndexes = {
      ...
      monthlyPremium: getColumnIndex(['Monthly Premium', 'MonthlyPremium', 'Premium']),
      commissionPercent: getColumnIndex(['Agent Commission %', 'Commission %', 'Commission Rate', 'Rate']),
      netCommission: getColumnIndex(['Net Commission', 'Net Commission Final', 'Commission Final']), // COLUMN AK
      coverageAmount: getColumnIndex(['Coverage Amount', 'Coverage', 'Loan Amount']),
      product: getColumnIndex(['Product', 'Product Type', 'Insurance Type']),
    }
    ...
    const data = calculationsRows.slice(1).map((row, rowIndex) => {
      ...
      // Get NET COMMISSION from column AK - THE FIX
      let netCommission = 0
      if (columnIndexes.netCommission !== -1 && row[columnIndexes.netCommission]) {
        netCommission = parseFloat(row[columnIndexes.netCommission].replace(/[^\d.-]/g, '')) || 0
      }
      obj._netCommission = String(netCommission)
